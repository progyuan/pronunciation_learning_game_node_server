...
var foo = function () { connect_and_test() };
...
var connect_and_maybe_test = function(test) {
...
function send_file_in_parts(f, n, logging) {
	...
	xhr.onload = function (reply) {
		...
		logging.innerHTML += "<br>" + timestamp() + " Server says ok!";	
		if (lastpacket) {
		    logging.innerHTML += "<br><b>Last packet sent:</b> Server returns <b>" + xhr.responseText.toString().replace(/,/g, ',<br>') +"</b>";
		    time_to_send_the_final_packet = false;
		    finalpacketsent=false;
		    packetn=0;
		}
