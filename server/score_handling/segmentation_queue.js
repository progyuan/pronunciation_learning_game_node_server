...
var classification_queue = [];
var classifier_busy = false;

var send_to_queue = function (handler) {

    console.log("classifier queue: ", classification_queue.length);

    if (!handler) 
	return false;

    classification_queue.push(handler);

    if (!classifier_busy) {
	debugout(  "Classifier free, process immediately for user "+handler.user);
	process_queue();
    }
    else
	debugout(  "Classifier busy, request from user "+handler.user+" is put to queue");
...
var process_queue = function() {
    ...
    console.log("Processing classification queue; Queue length", classification_queue.length);

    if (classification_queue.length > 0 ) {
	classifier_busy = true;

	var handler = classification_queue.shift();
	debugout(  "Classifier starts on "+handler.user, 1000, handler.user, handler.word_id);
	
	var ackbuffer = new Buffer(4);
	ackbuffer.writeInt32LE(-1);
	...
	var sizebuffer = new Buffer(4);
	sizebuffer.writeInt32LE(handler.payloadbuffer.length/4); // 'Cause this the bad programming is! We'll send the number of data points,
        // not the number of bytes we're sending!

	fs.readFile(conf.dnnconf.port_number_file, function(err, port) {

	    debugout(  "Connecting to port >"+port+"<", 0, handler.user);
	    
	    process.emit('user_event', handler.user, handler.word_id, 'timestamp',{name: 'dnn_start' }); 			
	    
	    var client = net.connect({port: ""+port},
				     function() { //'connect' listener
					 debugout('connected to server!', 0, handler.user);
		...
		debugout("Got "+Object.prototype.toString.call(data)+" of length "+ data.length+" in state "+state, 0, handler.user);
		
		if (state == "waitforacc") {
		    debugout("Got ack: "+data.readInt32LE(0), 0, handler.user);
		    client.write( handler.payloadbuffer );
		    state = "waitforreturnlen";
		}
		
		else if (state == "waitforreturnlen") {
		    var returnsizebuffer = new Buffer( new Int8Array(data) );
		    debugout( "Got data length: "+ returnsizebuffer.readInt32LE(0,4), 0, handler.user);
		    ...
		    debugout("Got returned data: ", 0, handler.user);
		    for (var i =0; i< returneddata.length; i+=4) {
			classes.push(returneddata.readFloatLE(i))
		    }
		    process.emit('user_event', handler.user, handler.word_id, 'timestamp',{name: 'dnn_stop' }); 			
		    debugout(handler.user + " wid" + handler.word_id +": Emitting classification done event", 100, handler.user, handler.word_id);
		    process.emit('user_event', 
				 handler.user, 
				 handler.word_id,
				 'classification_done', {
		...
		client.destroy();
		debugout('disconnected from classification server', 0, handler.user);
		...
		client.end();
		console.log("Timeout: Ending client connection");
		client.destroy();
		classifier_busy = false;
		process_queue;
	    });
...
module.exports = { send_to_queue :send_to_queue };
