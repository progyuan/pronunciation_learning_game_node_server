...
name_lex='clean-am/train.lex.utf8.selection'
name_ph='clean-am/siak_clean_a_utf.ph'
name_mc='clean-am/siak_clean_a.mc'
name_gcl='clean-am/siak_clean_a.gcl'
name_gk='clean-am/siak_clean_a.gk'
name_dur='clean-am/siak_clean_a.dur'


models_dir='clean-am/%s/' % targetword



#word_list='game_word_list.txt'

#Target_words=['_'] #'too','__']
#Target_words=['_','__','safe','too'];
#Target_words=['_','__','age','am','and','ant','arm','art','at','back','bag','bark','be','bee','big','buy']

#Target_words=[ line.strip() for line in open(word_list, 'r').readlines()   ]

Target_words=['_', '__', targetword]
...
def write_mc(States,name_mc):
	...
	for state in States:
		...
		#print state.mc_line
		mc_line=[]
		#print 'le', len(state.gaussian_idx_new),len(state.gaussian_idx_old)
		cnt_loop=0
		f_mc.write(str(state.num_gauss_per_state))
		f_mc.write(' ')
		#f_mc.write(str(state.new))
		for gaussian_idx_new in state.gaussian_idx_new:
			#print 'dbg',gaussian_idx_new,States[state.new].gaussian_idx_old[cnt_loop]
			#print 'dbg',gaussian_idx_new,States[state.new].gaussian_weight[cnt_loop]
			#line_to_write=[str(gaussian_idx_new) + ' ' 
			haista_paska=str(state.gaussian_weight[cnt_loop])#str(States[state.new].gaussian_weight[cnt_loop])
			#print haista_paska
			f_mc.write(str(gaussian_idx_new))
			line_to_write=str(gaussian_idx_new)+ ' ' + haista_paska
			#print 'line to write',line_to_write
			f_mc.write(' ')
			#print haista_paska
			f_mc.write(haista_paska)
			f_mc.write(' ')
			cnt_loop=cnt_loop+1
			a=0
		f_mc.write('\n')
                ...
                for line in f_dur:
                        ...
                        for state in states_mapped:
                                if state_idx_old==state.old:
                                        ...
                                        #print "in read_dur",state_idx_old,state.old,num_gauss_per_state
                                        #states_mapped[state.new].num_gauss_per_state=num_gauss_per_state
                                        #states_mapped[state.new].first_gaussian_idx_old= first_gaussian_idx_old
                                        #states_mapped[state.new].last_gaussian_idx_old= first_gaussian_idx_old+num_gauss_per_state
                                        #states_mapped[state.new].first_gaussian_idx_new =	first_gaussian_idx_new
                                        #states_mapped[state.new].last_gaussian_idx_new = first_gaussian_idx_new + num_gauss_per_state
                                        dur_line=line
                                        #print 'dur',dur_line
                                        ...
                                        #print states_mapped[state.new].dur_line	

			#print "in read_dur",states_mapped[state_idx].new
...
list_word_models=get_word_models_from_lex(Target_words,models_dir + targetword+'.lex',name_lex)
#	for triphone in target_word:
#		list_word_models.append(triphone)


print ("Word models list:")
print (list_word_models)
#for triphone in list_word_models:
#	print 'word_models',triphone

#list_word_models=['A-A+A','A-A+C']
#list_word_models=['_-A+A']
#list_word_models=['kkkkk']
print ("trifonilista", list_word_models)
...
write_phn(HMMs_idx_mapped, models_dir + targetword+'.ph',HMM_count)
...
write_mc(states_mapped, models_dir + targetword+'.mc')

#[states_mapped,cluster_idx_vec]=read_gcl(name_gcl,states_mapped)

#print cluster_idx_vec
#[clusters_mapped,cluster_idx_max]=find_clusters_mapping(cluster_idx_vec)

#states_mapped=cluster_idx_to_states_obj(clusters_mapped,states_mapped)

#states_mapped=read_gk_to_states(states_mapped,name_gk)

gaussians_mapped=read_gk(gaussians_mapped,name_gk)

write_gk(states_mapped, models_dir + targetword+'.gk',num_gauss_cumu,gaussians_mapped)
#print num_gauss_cumu

#states_mapped=cluster_idx_to_states_obj(clusters_mapped,states_mapped)

#write_gcl(states_mapped, models_dir + 'test.gcl',cluster_idx_max-1)
states_mapped=read_dur(name_dur,states_mapped)
write_dur(states_mapped,models_dir + targetword+'.dur')
